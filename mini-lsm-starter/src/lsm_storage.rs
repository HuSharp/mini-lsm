...
impl LsmStorageInner {
    ...
    pub fn get_with_txn(&self, key: &[u8], read_ts: u64) -> Result<Option<Bytes>> {
        let snapshot = self.state.read();
        ...
        ))); // memtable scan range is [key, key]
        ...
        let check_sst = |key: &[u8], sst: &SsTable| {
            ...
            if key_within(key, sst.first_key().key_ref(), sst.last_key().key_ref()) {
                // bloom filter check
        ...
        let memtable_and_l0_iter =
            TwoMergeIterator::create(memtable_iter, MergeIterator::create(l0_iters))?;
        ...
        let iter = LsmIterator::new(
            TwoMergeIterator::create(memtable_and_l0_iter, merge_level_iter)?,
            Bound::Unbounded,
            read_ts,
        )?;
    ...
    pub fn scan_with_txn(
        ...
        let two_merge_iter = TwoMergeIterator::create(memtable_and_l0_iter, merge_sst_iter)?;
