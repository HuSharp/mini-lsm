...
impl LeveledCompactionController {
    ...
    // contain exactly one SST in the upper level and overlapping SSTs in the lower level.
    ...
    // rely on https://github.com/facebook/rocksdb/wiki/Leveled-Compaction
    pub fn generate_compaction_task(
        &self,
        snapshot: &LsmStorageState,
    ) -> Option<LeveledCompactionTask> {
        // compute target level size
        // such as [0 0 30MB 300MB 3GB 30GB] when level_size_multiplier=10 and base_level_size_mb=200
        // because only keep at most one level below `base_level_size_mb`
        ...
        // directly place the SST from L0 to the lowest level possible
        ...
        // compute the compaction priorities of each level by `current_size / target_size`
        let mut compaction_priorities = Vec::with_capacity(self.options.max_levels);
        for level in 0..self.options.max_levels {
            let prio = real_level_size[level] as f64 / target_level_size[level] as f64;
            if prio > 1.0 {
                compaction_priorities.push((prio, level + 1));
            }
        }

        // select the compaction task with the highest priority
        compaction_priorities.sort_by(|a, b| a.partial_cmp(b).unwrap().reverse());
        if let Some((_, level)) = compaction_priorities.first() {
            println!(
    ...
    pub fn apply_compaction_result(
        ...
        let mut lower_level_sst_ids_set = task
            ...
            .copied()
            .collect::<HashSet<_>>();
        // None is for l0 compaction
        if let Some(upper_level) = task.upper_level {
            // remove the compacted SSTs from the upper level
        ...
        // add the new SSTs to the lower level
        new_lower_level_ssts.extend(new_ssts);
        // need to be sorted
